...
static struct argp_option opt_reverse[] =
	{
		{"shufFile", 'L', "<path>", 0, "Path to a .shuf file.\v"},
		{"outdir", 'o', "<path>", 0, "Output directory for recovered k-mer files.\v"},
		{"threads", 'p', "INT", 0, "Number of threads.\v"},
		{"byreads", 'b', 0, 0, "Recover k-mers from sketched reads.\v"},
		{0}};

static char doc_reverse[] =
	"\n"
	"Recover k-mers from sketches."
	"\v"
	"Input is a .co directory or compatible sketch directory.\n"
	"\n"
	"Examples:\n"
	"  kssd3a reverse -L default.shuf -o recovered sketch.co\n"
	"  kssd3a reverse --byreads -o recovered_reads reads.co";
...
{
	struct arg_reverse reverse = { 0, };
	int argc = state->argc - state->next + 1;
	char **argv = &state->argv[state->next - 1];
	char *argv0 = argv[0];

	reverse.global = state->input;
	argv[0] = malloc(strlen(state->name) + strlen(" reverse") + 1);

	if (!argv[0])
		argp_failure(state, 1, ENOMEM, 0);
	sprintf(argv[0], "%s reverse", state->name);
	argp_parse(&argp_reverse, argc, argv, ARGP_IN_ORDER, &argc, &reverse);
...
{
	unify_sketch_t *in_sketch = generic_sketch_parse(opt_val->remaining_args[0], SKETCH_PARSE_NONE);
	char (*gname)[PATHLEN] = in_sketch->gname;
	size_t file_size;
	int infile_num = in_sketch->infile_num;
	int kmerlen = in_sketch->kmerlen;
	char kstring[33];
	char rc_kstring[33];
	mkdir_p(opt_val->outdir);
	if (in_sketch->stat_type == 2)
	{
		...
		{
			uint64_t *nth_sketch_st = in_sketch->comb_sketch + in_sketch->sketch_index[gn];
			size_t nth_sketch_len = in_sketch->sketch_index[gn + 1] - in_sketch->sketch_index[gn];

			char *filename;
			(filename = strrchr(gname[gn], '/')) ? ++filename : (filename = gname[gn]);
			replaceChar(filename, ' ', '_');
			char fullname[PATHLEN];			
			sprintf(fullname, "%s/%s.kmer", opt_val->outdir, filename);			
			FILE *kmerf;			
			if ((kmerf = fopen(fullname, "w")) == NULL) err(errno, "%s", filename);

			for (int i = 0; i < nth_sketch_len; i++)
			{
				uint64_t unituple = reverse_reorder_unituple_by_coden_pattern64(nth_sketch_st[i]);
				unituple2kstring(unituple, kmerlen, kstring, rc_kstring);
				fprintf(kmerf, "%s\t%s\n",kstring, rc_kstring);
			}
			fclose(kmerf);
		}
	...
	else if (reverse_opt_val.byreads)
	{
...
int co_rvs2kmer_byreads(reverse_opt_val_t *opt_val)
{
	...
	int comp_code_bits = shuf_arr->dim_shuffle_stat.k - shuf_arr->dim_shuffle_stat.drlevel > COMPONENT_SZ
							 ? 4 * (shuf_arr->dim_shuffle_stat.k - shuf_arr->dim_shuffle_stat.drlevel - COMPONENT_SZ)
							 : 0;
...
int co_reverse2kmer(reverse_opt_val_t *opt_val)
{
	...
	for (int j = 0; j < co_qry_dstat.comp_num; j++)
	{
		...
		if ((cbd_fcode_comp_fp = fopen(co_cbd_fcode, "rb")) == NULL)
			err(errno, "co_reverse2kmer()::%s", co_cbd_fcode);
...
#pragma omp parallel for num_threads(p_fit_mem) schedule(guided)
	for (int k = 0; k < co_qry_dstat.infile_num; k++)
	{
		...
		FILE *kmerf;
		if ((kmerf = fopen(fullname, "w")) == NULL)
...
{
	// reverse component code used by wrt_co2cmpn_use_inn_subctx
	...
	llong unituple = (tuple & (half_outer_ctx_mask << half_outer_ctx_bits)) + ((tuple & half_outer_ctx_mask) >> inner_ctx_bits) + ((tuple & ((1LLU << inner_ctx_bits) - 1)) << half_outer_ctx_bits);
